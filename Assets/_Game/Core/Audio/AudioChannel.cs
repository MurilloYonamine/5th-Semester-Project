...
namespace FifthSemester.Core.Audio
{
    ...
    public class AudioChannel
    {
        ...
        public AudioChannel(int channel, AudioService audioService)
        {
        ...
        private IEnumerator VolumeLeveling()
        {
            while (ShouldContinueVolumeLeveling())
            {
                for (int i = _tracks.Count - 1; i >= 0; i--)
                {
                    ...
                    track.Volume = Mathf.MoveTowards(track.Volume, targetVol, AudioService.TRACK_TRANSITION_SPEED * Time.deltaTime);
